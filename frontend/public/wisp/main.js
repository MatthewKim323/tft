...
mesh.scale.set(1.2, 0.9, 1.2); // Wider horizontally, same height
mesh2.scale.set(1.2, 0.9, 1.2); // Match mesh2 scaling
group.add(mesh);
group.add(mesh2);
scene.add(group);
...
function render() {
  ...
  if (running || invalidate || isZoomingIn) {
    if (!invalidate) {
      time += dt * 0.03; // Slow down time by 97% for slower movement

      // Mouse-based rotation (disabled during zoom)
      if (!isZoomingIn) {
        targetRotationX = mouseY * 0.2; // More subtle effect
