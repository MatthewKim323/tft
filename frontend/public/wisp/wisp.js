...
// Function to return a fixed tft_bot purple color palette with subtle white hints.
function randomizePalette() {
  // TFT Bot purple-dominant palette with subtle white hints - matches the dashboard theme
  // #805AD5 (purple-3), #9F7AEA (purple-2), #B794F6 (purple-1), #C8A2C8 (light purple), #E9D5FF (very light), #F5F0FF (white hint)
  const fixedPalette = [0x805AD5, 0x9F7AEA, 0xB794F6, 0xC8A2C8, 0xE9D5FF, 0xF5F0FF];
  let colors = fixedPalette.map((v) => new Color().setHex(v)); // Convert hex values to Color objects.

  // Set background to pure black #1B1B1B to match tft_bot theme
  const bkg = new Color().setHex(0x1B1B1B);
...
vec3 getPoint(in vec3 p, out float n) {
  n = getHeight(p*1.);
  vec3 displaced = p + normalize(p) * n * 5.;
  // Scale horizontally (x and z) more than vertically (y)
  displaced.x *= 1.5; // Extend horizontally
  displaced.z *= 1.5; // Extend horizontally
  return displaced;
}`;
...
void main() {
  ...
  // Reduced multipliers for slower, more subtle movement
  pos.xyz = pos.xyz + .02 * normalize(curlNoise(pos.xyz * .1 + offset, time)) + .005 * vec3(snoise(pos.xy), snoise(pos.yz), snoise(pos.xz));
